-- ---------------------------------------------------------------------------
-- Invite a user to a Business, bypassing app_user RLS via a SECURITY DEFINER.
--
-- The user may not exist yet (inviting by phone). The policy on app_user.insert
-- has no business context, so we use a SECURITY DEFINER function that checks
-- the caller is a manager of the target business, then creates the user and
-- membership atomically (ADR 0016).
-- ---------------------------------------------------------------------------

create or replace function app.invite_user_to_business(
